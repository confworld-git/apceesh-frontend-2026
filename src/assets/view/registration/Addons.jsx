    ...
    <div className="py-6 px-4 md:px-6">
      ...
      <div className="flex justify-center mb-6">
        <button
          ...
          className="flex items-center gap-2 bg-blue-900 text-white px-8 py-3 rounded-xl font-semibold text-sm hover:bg-blue-800 transition-all duration-200 shadow-md shadow-blue-200 hover:shadow-lg"
        >
          <span>☰</span>
          {expanded ? "Hide" : "Add"} Addons
          {selectedAddons.length > 0 && (
            <span className="bg-white text-blue-900 text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center ml-1">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${selected
                    ? "border-blue-900 shadow-lg shadow-blue-100 scale-[1.02]"
                    : "border-gray-200 bg-white shadow-sm hover:border-blue-700 hover:shadow-md hover:scale-[1.01]"
                  }
                `}
              >
                {/* Coloured top strip */}
                <div className="bg-blue-900 px-4 pt-4 pb-5">
                ...
                <div className={`px-4 py-3 ${selected ? "bg-blue-50" : "bg-white"}`}>
                  ...
                  <p className={`text-lg font-extrabold ${selected ? "text-blue-900" : "text-gray-900"}`}>
                    ${addon.price}{" "}
        ...
        {selectedAddons.length > 0 && (
          <div className="mt-5 bg-blue-50 border border-blue-200 rounded-xl p-4">
              ...
              {selectedAddons.map((addon) => (
                <div key={addon.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-sm text-blue-900">+${addon.price}</span>
                    <button
                      type="button"
            ...
            <div className="mt-3 pt-3 border-t border-blue-200 flex justify-between">
              <span className="font-semibold text-sm text-blue-900">Add-ons Total</span>
              <span className="font-bold text-blue-900">+${totalAddonsAmount}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {!expanded && selectedAddons.length > 0 && (
        <div className="mt-3 flex items-center justify-between bg-blue-900 rounded-xl px-5 py-3 shadow-md shadow-blue-200">
          <div className="flex items-center gap-2">
            <span className="w-5 h-5 bg-white rounded-full flex items-center justify-center">
              <span className="text-blue-900 text-xs font-bold">{selectedAddons.length}</span>
            </span>
